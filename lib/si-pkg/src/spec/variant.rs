...
impl SchemaVariantSpec {
    ...
    fn merge_sockets_from(
        ...
        for this_socket in &self.sockets {
            ...
            // and continue.
            let other_socket = match other_spec
                .sockets
                .iter()
                .find(|sock| sock.name == this_socket.name && sock.kind() == this_socket.kind())
            {
                Some(other_socket) => other_socket,
                None => {
            ...
            let this_socket_maybe_func_unique_id = this_socket
                .data
                .as_ref()
            ...
            match (
                ...
                (None, Some(other_socket_func_unique_id)) => {
                    ...
                    if other_socket_func_unique_id != unset_func_spec_id {
                        ...
                        // If we did not find any input mismatches, we can just push the other
                        // socket into our merged sockets list. If we did find input mismatches, we
                        // need to process them and push the "this" socket into the list.
                        ...
                        // If the func is not the identity func, just push the "this" socket.
                        merged_sockets.push(this_socket.to_owned());
                    }
                }
                (Some(this_socket_func_unique_id), Some(other_socket_func_unique_id)) => {
                    if this_socket_func_unique_id == other_socket_func_unique_id
                        && (this_socket_func_unique_id == unset_func_spec_id
                            || this_socket_func_unique_id == identity_func_spec_id)
                    {
                        // If the sockets have funcs, both funcs are the same, and they are either
                        // identity or unset, then we push the "this" socket onto the list.
                        merged_sockets.push(this_socket.to_owned());
                    } else if other_socket_func_unique_id != identity_func_spec_id
                        || other_socket_func_unique_id != unset_func_spec_id
                    {
                        // Otherwise, if the "other" socket's func is neither identity nor unset,
                        // then we need to get input mistmatches and handle the merge skips in the
                        ...
                        // Handle the input matches in the same way as before.
                        if new_merge_skips.is_empty() {
                            merged_sockets.push(other_socket.to_owned());
                        } else {
                            merge_skips.extend(new_merge_skips.into_iter());
...
mod tests {
    ...
    fn test_schema_variant_merge() {
        ...
        let (merged_sv, skips) = sv_2.merge_prototypes_from(&sv_1, "identity", "unset");
