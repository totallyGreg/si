...
pub struct PoolNoodleConfig<S> {
    ...
    /// Max number of worker threads to run at once. Defaults to available_parallelism() or 32
    pub max_concurrency: u32,
    /// Maximum number of instances to manage at once
    pub pool_size: u32,
    /// Number of attempts to get from the pool before giving up with 100 ms between attempts
    pub retry_limit: u32,
    ...
    E: Send + Display + 'static,
{
                    ...
                    tokio::spawn(async move {
                        loop {
                            ...
                            tokio::select! {
                                ...
                                Some(task_type) = async { lq.pop() } => {
                                    me.handle_task(task_type).await;
                                }
    ...
    pub async fn get(&self) -> Result<LifeGuard<I, E, S>, E> {
        ...
        loop {
            ...
            if let Some(mut instance) = me.ready_queue.pop() {
                ...
                match &mut instance.ensure_healthy().await {
                    Ok(_) => {
                        ...
                        return Ok(LifeGuard::new(Some(instance), me.clone()));
                    }
                    Err(_) => {
    ...
    S: Spec,
{
    ...
    spec: S,
    work_queue: Queue<PoolNoodleTaskType<I, S>>,
}
    ...
    E: Send + Display + 'static,
{
    fn new(config: PoolNoodleConfig<S>) -> Self {
        ...
        Self {
            ...
            spec: config.spec,
            work_queue: Queue::new(config.max_concurrency as usize, 16),
        }
    }
